...
var bookCountList=[];
var bookMonthList=[];
function mainLibraryChart(path){
	 let today = new Date();   
	...
	$.ajax({
		type : 'get',
		dataType : "json",
		url : path + "/main/info/mainLibraryStatistics.do",
		success : function(res){
			totalList = res.totalList;
			bookCountList = res.bookCountList;
			bookMonthList = res.monthList;
...
function smartChartTwo() {
  ...
  var $revenueChartTwo = document.querySelector('#revenue-chart-two');
...
//               값 출력되는 구간
  ...
  revenueChartTwo = new ApexCharts($revenueChartTwo, revenueChartTwoOptions);
  revenueChartTwo.render();
