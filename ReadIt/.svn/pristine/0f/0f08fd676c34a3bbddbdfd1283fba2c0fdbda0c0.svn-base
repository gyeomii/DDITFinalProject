				...
				<div class="card">
					...
					<div class="card-body">
						<div class="row">
							...
							<input type="hidden" name="oq_no" value="${oftenQna.oq_no }" />
								<div class="mb-1">
									<label class="form-label" for="smallInput">제목</label> 
									<input id="title" class="form-control form-control-sm" type="text" name="oq_title" placeholder="Large Input" value="${oftenQna.oq_title}">
								</div>
								<div class="mb-1">
									<label class="form-label" for="smallInput">작성자</label>
									<input id="defaultInput" class="form-control form-control-sm" name="oq_writer" value="${oftenQna.oq_writer}" type="text" placeholder="Normal Input">
								</div>
								<div>
									<label class="form-label" for="largeInput">내용</label>
								</div>
								<div>
									<textarea class="textarea" name="oq_content" id="oq_content" rows="20" cols="90" placeholder="1000자 내외로 작성하세요." style="display: none;">${fn:escapeXml(oftenQna.oq_content)}</textarea>
								</div>
...
window.onload=function(){
	summernote_go($('#oq_content'),'<%=request.getContextPath()%>');
}
</script>
