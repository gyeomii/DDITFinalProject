...
<script>
	function initGo(){
		if(document.querySelector('#fp-default').value == ""){
			Swal.fire("예약불가","날짜를 선택해주십시오.")
			return;
		}
		if(document.querySelector('#seatTime').value == "0"){
			Swal.fire("예약불가","해당 일자는 만석입니다.")
			return;
		}
		if(document.querySelector('#seatTime').value == "0"){
			Swal.fire("예약불가","시간을 선택해주십시오.")
			return;
		}
		if(document.querySelector('#fr_capability').value == ""){
			Swal.fire("예약불가","이용 인원을 입력해주세요.")
			return;
		}
		if(document.querySelector('#fr_text').value == ""){
			Swal.fire("예약불가","이용 용도를 입력해주세요.")
			return;
		}
