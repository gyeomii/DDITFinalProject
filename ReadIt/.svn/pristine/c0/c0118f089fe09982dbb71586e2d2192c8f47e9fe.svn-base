						...
						<div class="row">
							...
							<input type="hidden" name="notice_no" value="${notice.notice_no }" />
								<div class="mb-1">
									...
									<input id="title" class="form-control form-control-sm" type="text" name="notice_title" placeholder="Large Input" value="${notice.notice_title}">
								</div>
								<div class="mb-1">
									<label class="form-label" for="smallInput">작성자</label>
									<input id="defaultInput" class="form-control form-control-sm" name="notice_writer" value="${notice.notice_writer}" type="text" placeholder="Normal Input">
								</div>
								<div>
									<label class="form-label" for="largeInput">내용</label>
								</div>
								<div>
									<textarea class="textarea" name="notice_content" id="notice_content" rows="20" cols="90" placeholder="1000자 내외로 작성하세요." style="display: none;">${fn:escapeXml(notice.notice_content)}</textarea>
								</div>
...
window.onload=function(){
	summernote_go($('#notice_content'),'<%=request.getContextPath()%>');
}
</script>
