				...
				<div class="card">
					...
					<div class="card-body">
						<div class="row">
							...
							<input type="hidden" name="ev_no" value="${event.ev_no }" />
								<div class="mb-1">
									<label class="form-label" for="smallInput">제목</label> 
									<input id="title" class="form-control form-control-sm" type="text" name="ev_title" placeholder="Large Input" value="${event.ev_title}">
								</div>
								<div class="mb-1">
									<label class="form-label" for="smallInput">작성자</label>
									<input id="defaultInput" class="form-control form-control-sm" name="ev_writer" value="${event.ev_writer}" type="text" placeholder="Normal Input">
								</div>
								<div>
									<label class="form-label" for="largeInput">내용</label>
								</div>
								<div>
									<textarea class="textarea" name="ev_content" id="ev_content" rows="20" cols="90" placeholder="1000자 내외로 작성하세요." style="display: none;">${fn:escapeXml(event.ev_content)}</textarea>
								</div>
...
window.onload=function(){
	summernote_go($('#ev_content'),'<%=request.getContextPath()%>');
}
</script>
