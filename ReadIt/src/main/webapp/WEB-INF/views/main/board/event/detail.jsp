...
<body>
	...
	<div class="container">
			<div class="card" style="margin-top: 30px; min-height: 600px; min-width: 750px;">
				...
				<div class="demo-inline-spacing" style="float: right;">
					<span class="badge bg-primary"> <i data-feather="star" class="me-25"></i> <span>이벤트</span></span>
				</div>
				<div style="float: right;">
					<h3 class="card-title" style="text-align: center; font-style: oblique;">${event.ev_title}</h3>
				</div>
				...
				<div class="card-body">
					<div class="d-flex" style="float: right; padding-right: 30px;">
						...
						<div class="author-info">
							...
							<small><a href="#" class="text-body">${event.ev_writer}</a></small>
							<span class="text-muted ms-50 me-25">|</span>
							<small class="text-muted">작성일</small>
							<small><a href="#" class="text-body"><fmt:formatDate value="${event.ev_regdate }" pattern="yyyy-MM-dd"/></a></small>
						</div>
					</div>
					<br/>
					<hr>
					<div class="author-info" style="padding: 40px; min-height: 400px;">${event.ev_content}</div>
					<div class="form-group col-sm-12">
							<div class="card card-outline card-success">	
								<div class="card-footer">
									<div class="row">
										<!-- 첨부파일 썸네일 -->
										<c:forEach items="${event.attachList }" var="attach">
										<div class="col-xl-2 col-md-4 col-sm-5">
											<div class="card bg-primary text-white h-75" style="cursor:pointer;" onclick="location.href='<%=request.getContextPath()%>/main/board/event/getFile.do?ano=${attach.ano }';">
												<div class="card-body">
					...
					<sec:authorize access="hasRole('ROLE_ADMIN')"> 	
						<button type="button" id="modifyBtn" class="btn btn-success" onclick="modify_go('${event.ev_no}');">수 정</button>
						<button type="button" id="removeBtn" class="btn btn-warning" onclick="remove_go('${event.ev_no}');">삭 제</button>
					</sec:authorize>	
	...
	<script>
		function modify_go(ev_no) {
			location.href = "modifyForm.do?ev_no=" + ev_no;
		}

		function remove_go(ev_no) {
			location.href = "remove.do?ev_no=" + ev_no;
		}
