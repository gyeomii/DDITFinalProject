			...
			<section id="flatpickr">
				<div class="card" style="margin: 20px;">
					<div class="card-header">
						<h2 class="card-title">회의실 1실</h2>
					</div>
					<div class="card-header">
						<h4 class="card-title">일시 선택</h4>
					</div>
					<div class="card-body">
...
<script type="text/x-handlebars-template" id="seatTime-template">
	{{#if multiOneList}}
		  {{#each multiOneList}}
      			<option value="{{fr_hour}}">{{fr_hour}}~{{fr_hour_end}}시</option>
 		  {{/each}}
	  {{else}}
		<option value="0">만석입니다.</option>
   {{/if}}
...
<script>
	...
	function initGo(){	
		if(document.querySelector('#fp-default').value == ""){
			Swal.fire("예약 불가","날짜를 선택해주십시오.");
			return;
		}
		if(document.querySelector('#seatTime').value == "0"){
			Swal.fire("예약 불가","해당 일자는 만석입니다.");
			return;
		}
		if(document.querySelector('#seatTime').value == "0"){
			Swal.fire("예약 불가","시간을 선택해주십시오.");
			return;
		}
		if(document.querySelector('#fr_capability').value == ""){
			Swal.fire("예약 불가","이용 인원을 입력해주세요.");
			return;
		}
		if(document.querySelector('#fr_text').value == ""){
			Swal.fire("예약 불가","이용 용도를 입력해주세요.");
			return;
		}
